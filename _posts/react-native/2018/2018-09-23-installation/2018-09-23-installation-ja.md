...
このブログページは`Deprecated`されました。もう修正も管理もしません。react-nativeのインストール方法に関しては新しいブログポストを確認してください。

- [マック(Mac)にreact nativeの開発環境を構築する方法]({{site.url}}/react-native/install-on-mac/)
- [ウィンドウズ(Windows)にreact nativeの開発環境を構築する方法]({{site.url}}/react-native/install-on-windows/)
...
![リアクトネイティブ(React Native) インストール - xcode](/assets/images/category/react-native/installation/react_native_xcode_preferences.jpg)

メニューで```Locations```を選択して、下の```Command Line Tools```を最新版を選択します。

![リアクトネイティブ(React Native) インストール - xcode](/assets/images/category/react-native/installation/react_native_xcode_preferences_configure_command_line_tools.jpg)
