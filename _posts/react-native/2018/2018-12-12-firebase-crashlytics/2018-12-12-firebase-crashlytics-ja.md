...
### ファイアベース(Firebase)設定
下のリンクを押してファイアベースコンソール(Firbase Console)に移動して自分のプロジェクトを選択します。

- ファイアベースコンソール(Firebase Console): [https://console.firebase.google.com/](https://console.firebase.google.com/){:rel="nofollow noreferrer noopener" target="_blank"}

左メニューで`Crasylytics`を選択します。

![ファイアベースコンソール(Firbase Console) Crashlytics メニュー](/assets/images/category/react-native/firebase-crashlytics/menu-crashlytics.jpg)

上のような画面が見えたら、`Set up Crashlytics`を選択します。

![ファイアベースコンソール(Firbase Console) Crashlytics step 1](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-1.jpg)

上のような画面が見えたら、`No, this app does not have any version of the Crashlytics SDK installed`を選択して`Next`を選択します。

![ファイアベースコンソール(Firbase Console) Crashlytics step 2](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-2.jpg)

上のような画面が見えたら`Go to Crashlytics docs`を選択します。そしたら新しいタブにCrashlyticsのインストール方法を説明するサイトがひらけます。そのサイトのタブは閉じます。まだ、ファイアベースコンソール(Firebase Console)で戻ってきたら下のような画面が見えます。

![ファイアベースコンソール(Firbase Console) Crashlytics step 3](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-3.jpg)

この状態で下記の内容を進めます。アプリに既に設定をした方はこの画面まで進めて、Crashlyticsが設定された状態でシミュレータを実行します。そしたら下記のようなダッシュボードが見えます。

![ファイアベースコンソール(Firbase Console) Crashlytics ダッシュボード](/assets/images/category/react-native/firebase-crashlytics/crashlytics-dashboard.jpg)
...
リアクトネイティブ(React Native)プロジェクトフォルダにある```ios/[AppName].xcworkspace```を選択してXcodeを実効します。

![xcode 実効](/assets/images/category/react-native/firebase-crashlytics/execute_xcode.jpg)

左にあるファイルエクスプローラと```TARGETS```で自分のプロジェクトを選択します。そして上のメニューで```Build Phases```を選択します。

![xcode build phases](/assets/images/category/react-native/firebase-crashlytics/build_phases.jpg)

Build Phasesタブで左上の```+```ボタンを押して```New Run Script Phase```を選択します。

![new run script menu on build phases](/assets/images/category/react-native/firebase-crashlytics/new_run_script.jpg)
...
![add Run Script](/assets/images/category/react-native/firebase-crashlytics/add_run_script.jpg)
...
![Firebase Console Crashlytics](/assets/images/category/react-native/firebase-crashlytics/firebase_crashlytics.jpg)
...
![Firebase Console Crashlytics android](/assets/images/category/react-native/firebase-crashlytics/firebase_crashlytics_android.jpg)
