...
### 파이어베이스(Firebase) 설정
아래에 링크를 눌러 파이어베이스 콘솔(Firbase Console)로 이동하고 자신의 프로젝트를 선택합니다.

- 파이어베이스 콘솔(Firebase Console): [https://console.firebase.google.com/](https://console.firebase.google.com/){:rel="nofollow noreferrer noopener" target="_blank"}

왼쪽 메뉴에서 `Crasylytics`를 선택합니다.

![파이어베이스 콘솔(Firbase Console) Crashlytics 메뉴](/assets/images/category/react-native/firebase-crashlytics/menu-crashlytics.jpg)

위와 같은 화면이 보인다면, `Set up Crashlytics`를 선택합니다.

![파이어베이스 콘솔(Firbase Console) Crashlytics step 1](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-1.jpg)

위와 같은 화면이 보이면 `No, this app does not have any version of the Crashlytics SDK installed`를 선택하고 `Next`를 선택합니다.

![파이어베이스 콘솔(Firbase Console) Crashlytics step 2](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-2.jpg)

위와 같은 화면이 보인다면 `Go to Crashlytics docs`를 선택합니다. 그러면 새 창에 Crashlytics를 설치하는 방법에 대한 사이트가 열립니다. 그 사이트는 창을 닫아줍니다. 다시 파이어베이스 콘솔(Firebase Console)로 돌아오면 아래와 같은 화면이 보입니다.

![파이어베이스 콘솔(Firbase Console) Crashlytics step 3](/assets/images/category/react-native/firebase-crashlytics/crashlytics-step-3.jpg)

이 상태에서 아래에 내용을 진행합니다. 앱에 설정을 먼저 하신 분들은 이 화면까지 진행하신 후, Crashlytics가 설정된 상태에서 시뮬레이터를 실행합니다. 그러면 아래와 같이 대시보드가 성공적으로 보입니다.

![파이어베이스 콘솔(Firbase Console) Crashlytics 대시보드](/assets/images/category/react-native/firebase-crashlytics/crashlytics-dashboard.jpg)
...
![xcode 실행](/assets/images/category/react-native/firebase-crashlytics/execute_xcode.jpg)
