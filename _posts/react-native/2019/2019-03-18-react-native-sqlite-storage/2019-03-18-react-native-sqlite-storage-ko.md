...
## 개요
RN(React Native)로 앱을 개발하다보면 로컬 데이터베이스(DB)를 사용할 경우가 있습니다. 예를 들어 제가 개발한 단어장 앱([일단공부(일본어 단어 공부) - JLPT 단어 공부]( https://dev-yakuza.posstree.com/app/blaboo/en/){:target="_blank"})은 단어를 서버에서 받아오는게 아니라 앱과 함께 배포하고 있습니다. 이 블로그에서는 이렇게 앱내에서 데이터베이스(DB)를 사용하는 방법에 대해서 알아보겠습니다.
...
![RN(react-native) sqlite 툴](/assets/images/category/react-native/2019/react-native-sqlite-storage/sqlite-tool.jpg)

왼쪽 위에 `New Database`를 선택하여 새로운 `TestDB`라는 이름으로 데이터베이스를 생성합니다.

![RN(react-native) sqlite 툴 - DB 생성](/assets/images/category/react-native/2019/react-native-sqlite-storage/sqlite-tool-create-db.jpg)

아래와 같이 `test`라는 이름의 테이블을 생성하고 `id`, `name`, `age`, `email` 필드를 추가합니다.

![RN(react-native) sqlite 툴 - DB 테이블 생성](/assets/images/category/react-native/2019/react-native-sqlite-storage/sqlite-tool-create-db-table.jpg)
...
![RN(react-native) sqlite 툴 - DB 데이터 추가](/assets/images/category/react-native/2019/react-native-sqlite-storage/sqlite-insert-data.jpg)
...
![RN(react-native) sqlite 툴 - DB 데이터 확인](/assets/images/category/react-native/2019/react-native-sqlite-storage/sqlite-check-data.jpg)
...
![react-native-sqlite-storage 설정 완료](/assets/images/category/react-native/2019/react-native-sqlite-storage/setting.jpg)
...
1. `ios/[project name]/www` 폴더를 만들고 미리 생성한 sqlite DB를 복사합니다.
  ![react-native-sqlite-storage www 폴더 생성 및 파일 복사](/assets/images/category/react-native/2019/react-native-sqlite-storage/www_folder.jpg)
...
1. 왼쪽 상단에 `[project name]` 하단의 `[project name]` 폴더를 우클릭하고, `Add Files to [project name]`을 선택합니다.
  ![react-native-sqlite-storage xcode에 DB 파일 추가](/assets/images/category/react-native/2019/react-native-sqlite-storage/add_file_for_ios.jpg)
...
1. `android/app/src/main/assets/www` 폴더를 만들고 미리 생성한 sqlite DB를 복사합니다.
  ![react-native-sqlite-storage android에 DB 파일 추가](/assets/images/category/react-native/2019/react-native-sqlite-storage/www_folder_android.jpg)
