...
사진을 찍거나, 카메라롤에서 사진을 가져오기 위해 `react-native-image-picker` 라이브러리를 사용합니다. 자세한 내용은 아래에 블로그 포스트를 확인하세요.

- 블로그 포스트: [react-native-image-picker]({{site.url}}/{{page.categories}}/react-native-image-picker/){:target="_blank"}

react-native-image-picker 라이브러리를 사용하여 가져온 이미지를 react-native-fs를 사용하여 복사합니다.
...
웹 뷰에서도 복사한 이미지를 바로 불러와서 사용할 수 있습니다.
...
현재 iOS 디바이스에서 이미지가 표시되지 않는 버그가 있습니다. (아니면, 제가 방법을 잘 모르는 걸 수 있습니다...혹시 표시되시는 분은 방법을 좀 공유해주세요...)
