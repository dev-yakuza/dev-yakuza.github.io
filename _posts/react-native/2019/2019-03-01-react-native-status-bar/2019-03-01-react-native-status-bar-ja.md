...
## Splashスクリーン
私はリアクトネイティブ(React Native)でSplashスクリーンを使うため```react-native-splash-screen```を使ってます。```react-native-splash-screen```について詳しく内容は下記のブログを参考してください。
...
上のようにリアクトネイティブ(React Native)のStatusBarを使って```backgroundColor={'transparent'}```と```translucent={true}```を設定したら透明なStatusBarを使えることができます。
