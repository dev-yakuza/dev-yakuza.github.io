...
RN(React Native)는 기본적으로 Splash 스크린을 제공합니다. 이전 블로그에서도 소개해 드렸지만 `react-native-make` 툴을 이용하면 간단하게 다양한 크기에 Splash 이미지를 만들 수 있습니다. 아래에 링크는 `react-native-make` 툴를 사용하여 Splash 이미지를 만드는 방법에 관한 블로그입니다.

- [React Native에서 App 아이콘 & Splash 이미지 만들기]({{site.url}}/{{page.categories}}/react-native-make/){:target="_blank"}
