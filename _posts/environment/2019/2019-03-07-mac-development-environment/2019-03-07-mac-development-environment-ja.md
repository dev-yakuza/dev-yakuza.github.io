...
- [HomebrewとShell Scriptを使ってマック(Mac)の開発環境構築を自動化する]({{site.url}}/{{page.categories}}/configure-development-environment-on-mac-with-homebrew-and-shell-script/){:target="_blank"}
...
下記のコマンドで`pip`をインストールします。

```bash
sudo easy_install pip
```

パイソン(python)の仮想環境(Virtual Environment)の管理をサーポートしてくれるモジュールである`virtualenv`を下のコマンドでインストールします。

```bash
pip install virtualenv pylint autopep8
```

zsh設定ファイルに下記のように設定して仮想環境(Virtual Environment)に接続してるか確認できるように設定します。
