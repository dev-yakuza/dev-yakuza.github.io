...
![refresh token](/assets/images/category/laravel/jwt-refresh-token/refresh_token.jpg)

jwtトークンの有効期限が終ったり、以前のjwtトークンを使ったら下記のように```401```エラーの応答(Response)を確認することができます。

![fail to refresh token](/assets/images/category/laravel/jwt-refresh-token/fail_to_refresh_token.jpg)
