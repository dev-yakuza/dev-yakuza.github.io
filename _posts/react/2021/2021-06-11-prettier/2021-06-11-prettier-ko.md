...
description: 'React에서 Prettier를 사용하여 코드의 포멧을 일정하게 유지시켜 보자.'
...
혼자서 서비스를 개발할 때는 코드 포멧(스페이스, 탭, 쿼테이션 등)은 큰 문제가 되지 않습니다. 하지만, 여러 사람들과 함께 협업하여 서비스를 개발할 때, 각 개발자가 사용하는 코드 포멧이 다를 수 있습니다. 이렇게 코드 포멧이 다르면 코드가 읽기 어려워지며, 예상치 못한 버그등이 발생할 수 있습니다.

Prettier는 코드 포멧터(Code formatter)로써 미리 코드 포멧을 정의하고, 해당 포멧에 맞게 코드를 수정해 주는 역할을 합니다.

이번 블로그 포스트에서 React에서 Prettier를 사용하여 코드 포멧을 통일 시키는 방법에 대해서 알아보겠습니다.

## 프로젝트 준비

React의 Prettier를 사용하기 위해, `create-react-app`으로 간단하게 프로젝트를 생성하겠습니다. `create-react-app`에 관해 잘 모르시는 분들은 아래의 링크를 참고하시기 바랍니다.
...
React에서 Prettier를 사용하기 위해서는 사용할 코드 포멧을 미리 정의할 필요가 있습니다. 코드 포멧을 미리 정의하기 위해, `.prettierrc.js` 파일을 생성하고 다음과 같이 수정합니다.
...
다음 명령어를 사용하면 우리가 정의한 Prettier의 옵션을 사용하여 코드 포멧팅을 할 수 있습니다.

```bash
prettier --write .
```

명령어를 실행하면 다음과 같이 포멧팅이 진행된 파일 리스트를 확인할 수 있습니다.
...
이렇게 파일 포멧팅이 완료된 후, 다시 다음 명령어를 실행하여 코드 포멧을 확인해 보면

```bash
npx prettier --check .
```

다음과 같이 모든 파일들이 잘 포멧팅된 것을 확인할 수 있습니다.
...
이렇게 설정하였다면, VSCode에서 파일을 수정하고 저장하면, 자동으로 코드가 포멧팅되는 것을 확인할 수 있습니다.

## 완료

이것으로 React 프로젝트에서 Prettier를 설정하는 방법에 대해서 알아보았습니다. Prettier를 통해 잘 포멧팅된 코드를 관리해 보시기 바랍니다.
