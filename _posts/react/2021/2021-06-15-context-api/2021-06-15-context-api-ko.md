...
React에서 컴포넌트가 데이터를 다루는 방법으로 `Props`, `State` 그리고 `Context`가 있습니다. 이번 블로그 포스트에서는 Context에 관한 개념과 사용 방법을 정리해 보도록 하겠습니다.
...
이렇게 추가한 createContext를 사용하여 Context를 생성합니다. 이때, 전역 변수로 사용될 데이터의 초기값을 설정해야 합니다.
...
이것으로 React에서 컴포넌트가 전역 데이터를 다루는 방법인 Context에 대해 간단히 살펴보았습니다. 이번 블로그 포스트의 예제에서는 Context를 사용하지 않을 정도로 작은 프로젝트였기 때문에, Context를 사용하는 필요성을 못 느끼셨을거 같습니다. 하지만, 많은 컴포넌트를 사용하는 프로젝트에서는 Context를 사용하여 데이터를 공유하는 경우가 많으므로 이번 기회에 잘 공부해 두시면 도움이 될거 같습니다.
