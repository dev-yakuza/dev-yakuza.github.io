...
문제없이 Typescript를 설정하였다면 다음과 같이 React 프로젝트를 브라우저에서 실행되는 것을 확인할 수 있습니다.
...
create-react-app을 사용하는 이유는 React 프로젝트를 실행할 때, 많은 설정들을 하지 않기 위함인데, Typescript만을 위해 너무 많은 설정을 했습니다. 하지만 Typescript는 이제 Javascript에서 중요한 역할을 하고 있기 때문에 Typescript의 사용을 하지 않을 수 없습니다.

create-react-app도 이렇게 Typescript에 중요성을 인지하고 있고, 이를 더욱 간단하게 지원하기 위해 `Template` 옵션을 제공하고 있습니다. 그럼 create-react-app의 `Tempate` 옵션을 사용하여 React의 Typescript 프로젝트를 생성해 보도록 합시다.
...
이번 블로그 포스트에서는 `create-react-app`으로 생성한 React 프로젝트에 `Typescript`를 적용하는 방법에 대해 알아보았습니다. 또한 새롭게 Typescript가 적용된 React 프로젝트를 생성할 때, create-react-app의 `Template` 옵션을 활용하여 더욱 간단하게 Typescript가 적용된 React 프로젝트를 생성하는 방법도 알아보았습니다.
