...
테스트 코드 작성을 위해 Jest의 `it` 함수를 사용하며, `describe`와 마찬가지로 설명문과 함께 테스트 코드가 작성된 콜백 함수를 지정한다.
...
이렇게 파일을 수정하고 저장하면 앞에서 실행한 `jest --watch`에 의해 Jest가 변경된 파일을 인식하고 자동으로 테스트를 실행하여 다음과 같은 결과를 표시하는 것을 알 수 있다.
