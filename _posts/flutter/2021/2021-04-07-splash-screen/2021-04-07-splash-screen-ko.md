...
description: Flutter를 이용하여 앱을 개발해 봅시다. 이번 블로그 포스트에서는 Flutter에서 스플래시 스크린을 변경하는 방법에 대해서 알아봅시다.
...
하지만 `flutter_native_splash` 패키지를 사용하면, 앱 아이콘을 좀 더 쉽게 변경할 수 있습니다.
...
flutter_native_splash 패키지를 사용하기 위해서는 flutter_native_splash 패키지를 설치할 필요가 있습니다.

flutter_native_splash 패키지를 설치하기 위해 `pubspec.yaml` 파일을 열고 다음과 같이 수정합니다.
...
이렇게 수정하였다면, 다음 명령어를 통해 flutter_native_splash 패키지를 설치합니다.
...
이제 스플래시 스크린으로 생성할 이미지 파일을 설정할 필요가 있습니다. `pubspec.yaml` 파일을 열고 다음과 같이 수정합니다.
...
다음은 스플래시 스크린을 다음의 팁들과 함께 사용하면 좀 더 유용합니다.
