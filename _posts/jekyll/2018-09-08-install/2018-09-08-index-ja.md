...
## エラー対応
新しマック(Mac)へ既存のjekyllブログの環境を設定するためjekyllをインストールして`gem install`で必要なファイルをインストールする時エラーが発生しました。使ってるプラグインの中でJavaが必要なプラグインがあるみたいです。Javaをインストールしたら大丈夫になりました。もしプラグインをインストールする中、エラーが発生する方はJavaがインストールされたか確認してみてください。
